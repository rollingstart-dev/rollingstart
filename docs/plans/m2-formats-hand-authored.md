...
| Operations are `db:*` rituals, not stack lifecycle | The Rallly example maps `db:generate` / `db:deploy` / `db:seed` / `db:reset` (destructive), not `docker:up` | The roadmap scopes M2's operations to Rallly's `db:*` scripts, and the design's examples are all database rituals. Whether a stack-up script may ever be an operation brushes the never-orchestrates seam — deferred below until an operation consumer exists. `db:deploy` stands where this row first said `db:migrate` (2.1.3): `prisma migrate dev` prompts for a migration name and refuses a non-interactive run, with no flag to disarm it — the trap the `--force` rule exists to avoid. |
